...
public struct Commit: ObjectType, Hashable {
    ...
    public func trailers() -> Result<[Trailer], NSError> {
        var trailerArray = git_message_trailer_array()
        defer { git_message_trailer_array_free(&trailerArray) }

        let trailersResult = git_message_trailers(&trailerArray, message)
        guard trailersResult == GIT_OK.rawValue else {
            let err = NSError(gitError: trailersResult, pointOfFailure: "git_message_trailers")
            return .failure(err)
        }

        let trailers = UnsafeBufferPointer(
            start: trailerArray.trailers,
            count: trailerArray.count
        ).map(Trailer.init)
